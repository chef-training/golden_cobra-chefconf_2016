...
execute '/usr/local/bin/pip3 install django'
execute '/usr/local/bin/pip3 install uwsgi'
execute '/usr/local/bin/pip3 install gunicorn'
...
search('site','*:*').each do |site_data|
  ...
  execute "gunicorn #{site_name}.wsgi -D -b #{site_bind}" do
    cwd "/sites/#{site_name}"
    not_if { "ps aux | grep '/usr/local/bin/gunicorn' | grep '#{site_name}'" }
  end
